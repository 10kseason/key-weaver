# KeyWeaver Algorithm Lock v0.6.5

This document freezes the KeyWeaver v0.6.5 low-default whole-board algorithm contract.
...
The v0.6.5 contract is guarded by the unit tests named:

- `auto-more expansion reports larger budget`
- `preserve tap plus low caps high-key growth`
...
The release package also guards the CLI default with a dry-run sample report that must show:

- `"algorithmVersion": "v0.6.5"`
- `"expansionPolicy": "preserve-tap-plus-low"`

## Change Control

Changing any of the following requires an algorithm-lock update:

- Default high-key auto policy.
...
build\keyconv_gui.exe --smoke samples\simple_4k.osu dist\gui_smoke_065
