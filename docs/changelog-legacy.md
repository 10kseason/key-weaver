# KeyWeaver Legacy Changelog

This file preserves detailed release notes for versions before 1.1. Keep current and upcoming release notes in the top-level CHANGELOG.md, and move older detailed sections here when they stop being active release context.

## 1.0.1 - 2026-06-07

- Reworked the Windows GUI into a simplified conversion console with files, recipe, run, summary, and log panels.
...
- Enabled experimental NK2 for CLI/GUI batch conversion, including the GUI Algorithm selector path.
...
- Changed GUI Batch to operate on folders or multi-chart drops instead of treating one selected `.osu` as a batch.
...
- Added percent-left output to CLI and GUI batch progress lines.
