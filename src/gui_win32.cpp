...
void drawLogo(HDC dc, const AppState& state) {
    ...
    drawUiText(dc, state.smallFont, RGB(226, 235, 237), L"v1.1", version);
...
void paintUiChrome(AppState& state, HDC dc) {
    ...
    drawUiText(dc, state.smallFont, RGB(190, 205, 217), L"v1.1 conversion console",
               RECT{56, 66, 320, 88});
...
void createUi(AppState& state) {
    ...
    state.sourceEdit = makeControl(state, L"EDIT", L"", ES_AUTOHSCROLL, 714, 202, 72, 28, kEditSource,
                                   WS_EX_CLIENTEDGE);
    makeControl(state, L"STATIC", L"auto", SS_CENTER, 794, 206, 48, 22, -1);
...
int runSmoke(const std::filesystem::path& input, const std::filesystem::path& outputDir) {
    ...
    if (nk2BatchStreamedOutput.find("Progress: 100% done, 0% left, 0 left") == std::string::npos) {
